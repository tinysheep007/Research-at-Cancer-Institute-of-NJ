...
def create_overlap_matrix(sample_files, tss_data):
    ...
    for sample_file in sample_files:
        ...
        # Calculate overlaps for the current blood sample
        counts = []
        for _, row in tss_data.iterrows():
            start, end = row['start'], row['end']
            count = len(blood_sample[(blood_sample['Start_Position'] >= start) & (blood_sample['End_Position'] <= end)])
            counts.append(count)
...
# notes:
# from cluster to find out how much n-feature we need
# redo using midpoints
# heat map
# clustering on rows / col
# normalization
